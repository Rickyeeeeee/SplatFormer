...
def loss_mixing(schedule="linear"):
    valid_schedules = ("linear", "free-range-gs")
...
def get_loss_mix_weights(t, schedule):
    ...
    raise ValueError(
        f"Unsupported loss_mixing.schedule={schedule!r}; "
        "expected one of ('linear', 'free-range-gs')"
    )
...
def main(argv):
    ...
    # This mix variant always predicts and supervises every Gaussian attribute.
    ...
    print(
        f"GSFM-mix scene={scene['scene_name']} idx={scene['idx']} "
    ...
    for step in pbar:
        ...
        if step % log_interval == 0:
            ...
            logger.info(
                ...
                f"sampled_views={len(pred_imgs)} loss_type=x1_pred_mse "
