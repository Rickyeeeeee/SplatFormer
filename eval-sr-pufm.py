...
def _load_pufm_lib():
    lib_path = Path(__file__).with_name("overfit-sr-pufm.py")
    spec = importlib.util.spec_from_file_location("overfit_sr_pufm_lib", lib_path)
...
def main(argv):
    ...
    flow_cfg = pufm._resolve_flow_cfg()
    pufm.set_seed()
    ...
    dataset = pufm._build_dataset()
    scene_idx = pufm._find_scene_index(dataset, FLAGS.scene_name)
    ...
    eval_payload = pufm._build_split_payload(
        dataset,
        scene["idx"],
        scene["scene_name"],
        target_factor_entry,
    ...
    source_flow_gs = pufm.raw_to_flow_gs(input_gs_raw, flow_cfg["flow_space"])
    ...
    logger.info(
        "Evaluating checkpoint=%s scene=%s idx=%s flow_space=%s flow_steps=%s",
        checkpoint_path,
        scene["scene_name"],
        scene["idx"],
        flow_cfg["flow_space"],
    ...
    for idx, flow_steps in enumerate(eval_steps):
        ...
        metrics, metrics_input = pufm.evaluate_single_scene(
            model=model,
